...
if ($.isNode()) {
  ...
  cookiesArr.reverse();
  cookiesArr.push(...[$.getdata('CookieJD3'),$.getdata('CookieJD2'), $.getdata('CookieJD')]);
  cookiesArr.reverse();
  ...
  if ($.getdata('jd3_joy_invite_pin')) {
    if (invite_pins.length > 0) {
      invite_pins.push($.getdata('jd3_joy_invite_pin'))
    } else {
      invite_pins = [];
      invite_pins.push($.getdata('jd3_joy_invite_pin'));
    }
  ...
  if ($.getdata('jd3_joy_run_pin')) {
    if (run_pins.length > 0) {
      run_pins.push($.getdata('jd3_joy_run_pin'))
    } else {
      run_pins = [];
      run_pins.push($.getdata('jd3_joy_run_pin'));
    }
